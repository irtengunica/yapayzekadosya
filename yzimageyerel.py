...
for item in dosyalistesi:
 ...
 image = cv2.imread(kaynakklasor+dosyaAdi)
...
#  print('??????')
 ...
 cerceveadi =Cerceve(frame_id)# Cerceve(frame_id)
 goderilecekveri=json.dumps(cerceveadi, default=cerceveadi.cerceve2dict)
 print(goderilecekveri)
 if os.path.isfile("karaelmas/test200.json"):
  with open('karaelmas/test200.json', 'a') as f:
   f.write(goderilecekveri)
   f.write('\n ')
 else:
